...
1. **Ethernet3 on leaf1 is administratively disabled** (`link_status: disabled`, `protocol_status: down`), yet it carries a configured IP `10.100.1.1/24`.
   - Dependent evidence: no route for `10.100.1.0/24` appears in leaf1's own route table, nor in spine1, spine2, or leaf2's route tables — even though the structurally analogous interface on leaf2 (Ethernet3, `10.200.2.1/24`) is up and its subnet `10.200.2.0/24` is correctly advertised and present on spine1, spine2, and leaf1.

2. **A third BGP peer on leaf1 (remote AS 65999, peer address 10.1.9.9) is stuck in `Idle` state** with `idle_reason: "NoInterface"`, and 0 messages/prefixes exchanged.
   - Dependent evidence: none of leaf1's interfaces (Ethernet1 `10.1.1.0/31`, Ethernet2 `10.1.2.0/31`, Ethernet3 `10.100.1.1/24`, Loopback0 `10
