...
def _credentials() -> tuple[str, str]:
    ...
    if not username or not password:
        raise EapiError(
            "no_credentials",
            "EOS_USERNAME and EOS_PASSWORD must be set (see .env.example)",
        )
...
def run_commands(address: str, commands: list[str]) -> list[dict]:
    ...
    try:
        ...
        raise EapiError(
            "no_response",
            f"no response from {address} within {TIMEOUT_SECONDS}s",
        ) from exc
    except requests.exceptions.ConnectionError as exc:
        raise EapiError("connect_failed", f"could not connect to {address}: {exc}") from exc

    if response.status_code == 401:
        raise EapiError("auth_rejected", f"authentication rejected by {address}")
    ...
    if "error" in body:
        raise EapiError(
            "command_failed",
            f"{address} rejected a command: {body['error'].get('message')}",
        )
